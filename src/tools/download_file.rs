...
impl PortableTool for DownloadFileTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        // Authorize before writing; `Sandbox::write` resolves the destination
        // (so relative paths match absolute allow patterns) and creates parents.
        let sandbox = Sandbox::new(self.policy.clone());
        sandbox.write(path, &body)?;
