...
impl Tool for FileViewTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let result = md
            ...
            .map_err(|e| ToolExecError::Message(format!("conversion failed: {e}")))?;
        ...
        match result {
            Some(r) => {
                ...
                process_output(&r.text_content, None, None).map_err(ToolExecError::Message)
            }
            None => Err(ToolExecError::Message(
