...
- **External-command timeouts now enforced** — `ExtBuiltin` (external commands dispatched through bashkit) switched from blocking `std::process::Command::wait_with_output()` to async `tokio::process::Command` with `kill_on_drop`. Previously a long-running external command (e.g. `sleep 300`) ran to completion because the blocking wait never yielded to bashkit's `tokio::time::timeout`; now the child is killed at the configured timeout. This also stops blocking a tokio worker thread. New regression tests cover fast commands, exit-code propagation, and prompt process termination.
- **Test coverage expanded** — added unit tests for `config` (parsing, defaults, api-key resolution, resolved paths), `memory` (add/delete/persist), `util` (`fmt_bytes`, bars), `tools::process_output`/`truncate`/`fmt_offset_limit` (pagination, caps, error cases), `cli` (`prompt_text`, `is_interactive`, `is_vanilla`), and `policy::resolve_policy_pattern` (relative/absolute/home/dot-segment resolution).
