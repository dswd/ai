...
- **Date-aware explicit session names** — an undated `--session-name NAME` is an alias across dates: it resumes the most recently modified session named `NAME` or `YYYY-MM-DD_NAME`, and creates `YYYY-MM-DD_NAME` when none exists. A dated `--session-name YYYY-MM-DD_NAME` is matched exactly. `ai session delete NAME` resolves the same way.
- **Model-initiated exit** — in an interactive session the model gets an `exit_program` tool and can end the program when the user asks to quit or exit, running the normal end-of-session path (session save, resume hint). The `ai setup` AI gets the same tool so it can end setup on request. Once the tool runs, the in-flight model stream is aborted, so no further assistant output is generated or printed. It is not exposed to one-off runs.
...
- **`ai completions SHELL`** — generate a shell completion script for bash, zsh, fish, and the other supported shells, then exit.
...
- **SQLite-backed memory and transcript index** — persistent memory now lives in a SQLite database (`memory.db`, from the bundled SQLite) instead of a JSON file. `memory` holds distilled facts with tags and provenance (`origin`, `source_session`); `transcripts` holds derived user+agent Q&A tuples from sessions, with tool traffic dropped. Retrieval queries both tables and refreshes a `last_used` stamp used by maintenance. The same retrieval backs the `memory_search` tool, per-message prompt augmentation, and the new CLI commands. `rusqlite` (bundled) is a new dependency.
- **`ai memory list` and `ai memory search QUERY`** — inspect the memory store and search memory plus past conversation transcripts (kind, key, session/date, tags, text), then exit. Output is a colored header plus two-line entries on a terminal: the fact/result on one line, an indented metadata line (cyan tags, color-coded origin, blue session, grey dates) below; colors are dropped when piped or under `--no-color`/`NO_COLOR`.
- **`ai dream` memory maintenance** — a three-step offline routine run across a work pool (`ai dream --jobs N`, config `dream.jobs`, default 4): (1) *extract* — batched LLM requests over unprocessed transcript tuples (10 from one session per request) store durable facts via `memory_add`, then mark the tuples processed; (2) *prune* — delete processed transcripts older than 7 days; (3) *judge* — entries unused and unjudged for 30 days are reviewed with their similar-but-newer neighbours as evidence and hard-deleted via a batch-scoped `memory_delete` when obsolete. Every stored memory records where it came from.
- **Memory is on by default** — persistent memory uses the configured path (config `memory:`, default `<data-dir>/ai/memory.db`); `--no-memory` disables it for a run. `ai dream`, `ai memory list`, and `ai memory search` work without any flag.
- **Derived transcript index** — `Session::save` indexes the session's completed exchanges into the database (idempotent; rewrites changed rows and drops rows beyond a cleared or compacted log), and a one-time backfill indexes pre-existing session files. Transcript excerpts are injected into context inside the existing "data, not instructions" envelope, each labelled with its session and date and capped per prompt.
- **Capped memory search fragments** — search results and injected memory references longer than 100 characters are shown as a `…`-fenced fragment, capped at 100 characters including the ellipses. Applies to `ai memory search`, the `memory_search` tool, and per-message memory injection (interactive and one-shot); `ai memory list` still prints full entries.
- **Match scores in memory output** — retrieval hits carry a cosine similarity score (higher is better), shown in the per-message 🧠 injection log, `ai memory search`, and the `memory_search` tool.
- **`memory_get` tool** — retrieve a single memory entry by its key, returning the full text plus tags, origin, creation date, and source session, so a key referenced in search results or injected context can be expanded.
- **Semantic memory search (local embeddings + sqlite-vec)** — memory facts and transcript excerpts are embedded locally with `fastembed` (ONNX) and searched by vector similarity. `sqlite-vec`'s `vec0` tables are statically linked; retrieval is KNN with a cosine-distance cutoff (default `0.175` for E5 models, `0.45` otherwise, overridable with `memory_max_distance`), memory ranked ahead of transcripts. The embedding model is configurable via `embedding_model` (default `multilingual-e5-small`; E5 `query:`/`passage:` prefixes applied automatically) and downloads to the user cache dir on first use. `fastembed` and `sqlite-vec` are new dependencies; the build compiles the extension and ONNX Runtime and so needs network and a C toolchain.
- **End-of-session dream prompt and `dream.auto`** — when an interactive session ends with more than 50 pending maintenance tasks (unprocessed transcript tuples plus entries awaiting judgement), `ai` asks on the terminal whether to run memory maintenance before exiting (default No; a declined answer prints a reminder). Setting `dream.auto: true` runs maintenance automatically at the end of every saved interactive session, regardless of the task count. Both use the configured provider/model, block until done, and never fail the session exit; one-off `ai run`, piped-stdin, `--no-session`, empty, and `--no-memory` sessions are skipped.

### Changed

- **Breaking: `--init` renamed to `ai setup`** — no alias. `ai setup` creates or reconfigures the config; an existing file is backed up to `config.yaml.bak` first.
- **Breaking: `search.searxng_url` replaced by `search.providers`** — SearXNG is now a normal provider entry (`{ name: searxng, url: ... }`) in the ordered list. Configs still using `search.searxng_url` get an unknown-key warning (and fail strict parsing during `ai setup`); migrate by listing the provider.
...
- **Memory storage moved to SQLite** — the `memory` config path now names a SQLite database; the default filename is `memory.db`. An existing `memory.json` is migrated once on first open (entries, with `keywords` → tags, timestamps, origin, and source preserved) and renamed `memory.json.bak`. The memory tools now take `tags` instead of `keywords`, and `memory_add` accepts an `origin` (`user`/`agent`).
- **Memory distillation is now offline** — the per-run `reconcile_memory` pass (an extra LLM request after every interactive run) is replaced by `ai dream`. Until a dream runs, new facts remain retrievable from the transcript index rather than from distilled memory.
- **Breaking: CLI reworked into subcommands** — the flag-based commands are removed, with no aliases: `ai run "PROMPT"` for a one-shot (bare `ai` now starts an interactive session and `cat f | ai run` still works), `ai session list` / `ai session delete NAME`, `ai memory list` / `ai memory search QUERY`, `ai dream [--jobs N]`, `ai setup [FILE]`, and `ai completions SHELL`. `-s`/`--session-name NAME` now only names the session (requires a value, no longer starts one); the old `--session` long form is gone. `--probe-web` remains as a hidden `ai probe-web QUERY`.
- **Breaking: value flags accept space-separated values** — `require_equals` was dropped, so `-r .`, `--model gpt-4o`, `-s NAME`, etc. work alongside the existing `-r=.` / `--model=gpt-4o`. The optional-value flags (`-X/--container`, `--thinking`) still accept both forms but take the next bare token as their value, so `ai run "p" -X` is the way to request the default image.
- **Breaking: `-m`/`--memory` removed** — the memory database path is now config-only (`memory:` in config.yaml; default `<data-dir>/ai/memory.db`). `--no-memory` still disables reading and writing memory.
...
- **FTS5 keyword index** for `memory` and `transcripts` — superseded by semantic (embedding) search.

- **Inline memory reconciliation** and the session `reconciled_until` field — superseded by `ai dream`.

### Fixed

- **Weak memory matches were injected into every conversation** — retrieval had no relevance floor, so a fact could be injected whenever the message shared a single common word with it (and 1-character tokens from contractions like `let's` matched as terms). A hit now must share at least two distinct query terms with the entry's text or tags; single-term queries still match on that term, and query terms shorter than two characters are ignored. Applies to automatic injection and the `memory_search` tool.
...
- **Session-name path traversal** — `--session-name=../../x` and `session delete ../foo` could read/delete files outside the session directory; names are now validated to a single path component.
...
- **`~` expansion** — `~`/`~/…` is now expanded for `--config`, `--policy`, `--skill`, config paths (`session_dir`, `skills_dir`, `policy`, `memory`), and the `ai setup FILE` argument, matching the documented examples.
...
- **`ai session list` output** — now shows each session's provider and reports sessions that cannot be parsed instead of printing only the name.
