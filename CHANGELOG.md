...
- **External-command sandbox (Linux Landlock)** — `-x` now runs external commands through a self-launcher (`ai --sandbox-exec -- <cmd>`) that applies a Landlock ruleset derived from the policy (granted read/write roots plus a base system-access set, and TCP bind/connect denial when no web grant exists) and resource limits before `execvp`; children inherit the confinement. `--sandbox=auto|on|off` controls it (`auto` warns and falls back on unsupported platforms, `on` fails). The translation is intentionally coarse — Landlock is allow-list only, so globs collapse to directory roots and deny rules cannot be represented, which is warned about at startup. `git_diff`/`git_log` are routed through the same sandbox.
