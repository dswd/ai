...
- **Model-initiated exit** — in an interactive session (`-s`) the model gets an `exit_program` tool and can end the program when the user asks to quit or exit, running the normal end-of-session path (memory reconciliation, session save, resume hint). It is not exposed to one-off runs or setup.
- **Informative session prompt** — the interactive prompt now shows the session name, the active container image when commands run in a container, and a colored five-cell context-window gauge with percent, e.g. `2026-09-15_calm-hawk │ ⬢ debian │ ▰▰▰▰▱ 82% ❯`. Styling collapses automatically under `--no-color`/`NO_COLOR`.
