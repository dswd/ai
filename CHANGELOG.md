...
- **SQLite-backed memory and transcript index** — persistent memory now lives in a SQLite database (`memory.db`, FTS5 full-text search from the bundled SQLite) instead of a JSON file. `memory` holds distilled facts with tags and provenance (`origin`, `source_session`); `transcripts` holds derived user+agent Q&A tuples from sessions, with tool traffic dropped. Retrieval queries both tables and fuses them with reciprocal rank fusion (memory weighted above transcripts), refreshing a `last_used` stamp used by maintenance. The same retrieval backs the `memory_search` tool, per-message prompt augmentation, and the new CLI commands. `rusqlite` (bundled) is a new dependency.
...
- **Capped memory search fragments** — search results and injected memory references longer than 100 characters are shown as a `…`-fenced fragment centered on the matching query term, capped at 100 characters including the ellipses. Applies to `ai memory search`, the `memory_search` tool, and per-message memory injection (interactive and one-shot); `ai memory list` still prints full entries.
