...
- **Inline help via a `manual` tool** — the agent can look up `ai`'s own documentation (embedded `README.md`, `config.example.yaml`, a curated `docs/manual.md`, and generated CLI help) to answer questions about commands, flags, configuration, and behavior. Topic-indexed with a table of contents; `/help` now also points users to just ask the AI.
- **Expanded embedded manual** — `docs/manual.md` grew to ~1000 lines with detailed references for commands and flags, configuration, providers, policy, filesystem and containers, sessions, memory and `ai dream`, skills, web/search, MCP, the built-in tools, inline help, output and thinking, troubleshooting, and building from source.
...
- **Reusable approval rules from the permission prompt** — in ask mode, `r` opens a rule builder: choose allow/deny, edit the pre-filled subject (path, URL, query, or command; wildcards allowed), then choose whether to persist the rule to the policy file or keep it for the session only. Persisted rules are appended to the effective policy file (`--policy`, else `config.policy`, else a new default `<config-dir>/ai/policy`); the config file is never rewritten, and a failed write falls back to session-only with a warning. A finished rule that doesn't cover the triggering request warns and asks again.

### Changed

- **Breaking: ask prompt reworked** — `Allow <action> for <target>? [y=once, r=rule, N=deny]` replaces the old `a=always`/`r=this dir` shortcuts; to remember a target for the session now, create a rule and decline to persist it. `ai setup`'s config-save approval stays a plain yes/no.
- **Default policy path** — when neither `--policy` nor `config.policy` is set, `<config-dir>/ai/policy` (e.g. `~/.config/ai/policy`) is loaded if it exists, and it is the target for persisted approval rules.
- **Policy examples corrected** — `web fetch`/`web search` (with a space) are not accepted by the policy parser; the manual and README now use `web-fetch`/`web-search`.
- **Memory injection skips the current session** — per-message retrieval and the `memory_search` tool no longer return transcript excerpts from the session in progress, so a resumed or long session does not re-inject its own conversation. Other sessions' excerpts remain retrievable, and the over-fetch window widens to compensate. Maintenance (`ai dream`, including the end-of-session auto-dream) is unaffected: it searches and reviews every session, including the one just ended.
