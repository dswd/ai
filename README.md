...
- **Persistent memory** — Agent memory stored to disk (path configurable via `memory:` in the config) and injected into the system prompt; `--no-memory` disables it. Search results and injected references are capped to a 100-character fragment centered on the matching query term.
